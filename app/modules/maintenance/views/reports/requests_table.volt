    ...
    <div class="col-md-7 col-sm-7">
        <div class="dataTables_paginate paging_bootstrap_full_number" id="sample_1_paginate">
            <ul class="pagination" style="visibility: visible;">
                <li class='requests_reports_page'><a href="1">الأول</a></li>
                <li class='requests_reports_page'><a href="{{content['before']}}">السابق</a></li>
                <li class='requests_reports_page'><a href="{{content['next']}}">التالي</a></li>
                <li class='requests_reports_page'><a href="{{content['last']}}">الأخير</a></li>
            </ul>
        </div>
    </div>
</div>
<script>
    $(".requests_reports_page a").click(function(e) {
        e.preventDefault();
        var url = "{{url('maintenance_module/reports/requests_table')}}"+"&page="+$(this).attr("href"); // the script where you handle the form input.
        console.log("logged");
        $.ajax({
            type: "POST",
            url: url,
            data: $("#requests_reports_filter").serialize(), // serializes the form's elements.
            success: function (response)
            {

                $("#requests_req_report").html(response);
