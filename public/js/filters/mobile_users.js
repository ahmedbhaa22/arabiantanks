$(document).ready(function() {
    $('#sample_1 tfoot th.filter').each( function () {
     ...
     var table = $('#sample_1').DataTable({
        ...
        ajax: {
            url: main_url+'cms_module/mobile_users/ajaxContent',
            method: 'POST'
        },
    ...
    function createBtn(data) {
        return '<div class="btn-group" style="position:relative">' +
            ...
            '<a href="'+main_url+'cms_module/mobile_users/edit/' + data + '" data-toggle="modal"><i class="icon-tag"></i> تعديل </a></li>' +
            '<li><a onclick="return confirm_delete()" href="'+main_url+'cms_module/mobile-users/delete/' + data + '"><i class="icon-user"></i> حذف </a></li></ul></div>';
    }
    ...
    $( ".delete-btn" ).on( "click", function() {
        ...
        $.ajax({
            type        : 'post', // define the type of HTTP verb we want to use (POST for our form)
            url         : main_url+'cms_module/mobile_users/ajaxDelete', // the url where we want to POST
            data        : { checkboxes : checkedValues }, // our data object
